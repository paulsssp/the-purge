...
struct PLAYER_NAME : public Player {
  ...
  // bfs para IR hacia una barricada
  Pos bfs_path_barricade2 (const Pos& pos_ini, VVP& prev_path) {
    VVB visited(board_rows(), VB(board_cols(), false));
    visited[pos_ini.i][pos_ini.j] = true;
    queue<Pos> q;
    q.push(pos_ini);
  ...
  Pos bfs_path_atack_winner (const Pos& pos_ini, VVP& prev_path) {
    VVB visited(board_rows(), VB(board_cols(), false));
    visited[pos_ini.i][pos_ini.j] = true;
    queue<Pos> q;
    q.push(pos_ini);
    ...
    while (not q.empty()) {
      ...
      for (Dir d : dirs) {
        ...
        if (pos_ok(npos) and not visited[npos.i][npos.j] and valid_cell_for_attacking(npos)) {
          ...
          visited[npos.i][npos.j] = true;
          prev_path[npos.i][npos.j] = pos;
          if (cell(npos).id == w and has_worse_weapon(npos, pos_ini)) {
            return npos;
          }
        }
      }
  ...
  Pos bfs_path_atack_builder_winner (const Pos& pos_ini, VVP& prev_path) {
    VVB visited(board_rows(), VB(board_cols(), false));
    visited[pos_ini.i][pos_ini.j] = true;
    queue<Pos> q;
    q.push(pos_ini);
    ...
    while (not q.empty()) {
      ...
      for (Dir d : dirs) {
        ...
        if (pos_ok(npos) and not visited[npos.i][npos.j] and valid_cell_for_attacking(npos)) {
          ...
          visited[npos.i][npos.j] = true;
          prev_path[npos.i][npos.j] = pos;
          if (cell(npos).id == w and citizen(cell(npos).id).type == Builder) {
            return npos;
          }
        }
      }
      ...
      if (is_day()) {
        ...
        if (citizen(id).life < builder_ini_life()) {
          ...
          // implementar que si no hay comida, los dos guerreros están muertos y no tienen toda la vida, que vayan a las barricadas
            ...
            // si les falta un toque para morir, que vayan a las barricadas
            if (citizen(id).life <= 20) {
              Pos barricade = bfs_path_barricade2(citizen(id).pos, prev_path);
              if (barricade.i != -1) {
                Pos reverse = reverse_path(barricade, prev_path, id);
                Dir d = set_movement(reverse, id);
                move(citizen(id).id, d);
              }
              else {
  ...
  void move_warriors() {
    ...
    for (int id : w) {
      ...
      // que mate
        ...
        // si vamos por la mitad de la partida y no voy ganando, focuseo al jugador que va ganando
        if (double(round()) >= double(num_rounds())/2 and me() != winner()) {
          if (citizen(id).weapon == Hammer) {
            Pos enemy = bfs_path_atack_builder_winner(citizen(id).pos, prev_path);
            if (enemy.i != -1) {
            ...
            Pos enemy = bfs_path_atack_winner(citizen(id).pos, prev_path);
            if (enemy.i != -1) {
              Pos reverse = reverse_path(enemy, prev_path, id);
              Dir d = set_movement(reverse, id);
              move(citizen(id).id, d);
