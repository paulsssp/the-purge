    ...
    for (int i : b) {
      if (is_day()) {
        ...
        if (barricades(me()).size() < 3) {
          ...
          Pos future_barricade = bfs_path_barricades(citizen(i).pos, prev_path);
          if (future_barricade.i != -1) {
            Pos reverse = reverse_path(future_barricade, prev_path, i);
            Dir d = set_movement(reverse, i);
            if (citizen(i).pos + d == future_barricade) build(citizen(i).id, d);
            else move(citizen(i).id, d);
          }
        } 
        // si ya están todas que vayan a por dinero
        else {
          Pos money = bfs_path_bonus(citizen(i).pos, prev_path, Money);
          if (money.i != -1) {
            Pos reverse = reverse_path(money, prev_path, i);
            Dir d = set_movement(reverse, i);
            move(citizen(i).id, d);
          }
        }          
